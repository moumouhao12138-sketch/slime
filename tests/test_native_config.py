...
class NativeDispatchConfigTests(unittest.TestCase):
    ...
    def test_default_config_uses_a_general_capability_pool(self):
        ...
        self.assertEqual(payload["runtime"]["worker_healthcheck"], "startup_and_task")
        ...
        self.assertEqual(
            pi["env"]["PI_MODEL"],
            "${SLIME_PI_MODEL|SLIME_CODEX_MODEL|SLIME_LLM_MODEL}",
        )
        self.assertEqual(
            pi["env"]["PI_BASE_URL"],
            "${SLIME_PI_BASE_URL|SLIME_CODEX_BASE_URL|SLIME_LLM_BASE_URL}",
        )
        self.assertEqual(
            pi["env"]["PI_API_KEY"],
