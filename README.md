...
- **原生 Worker 适配**：默认启用 DeepSeek Harness；Codex、Pi 和 Claude 可按需启用，并为每个任务隔离会话上下文。
- **混合启动模式**：Agent Match 题目先由 Cairn Bootstrap 建立事实，再进入 Reason 和 Explore 分支。
...
默认应用镜像为 `ghcr.io/moumouhao12138-sketch/slime:0.0.42`，默认 Worker 镜像为 `ghcr.io/moumouhao12138-sketch/slime-worker:0.0.42`。
...
默认 DeepSeek Harness Worker 至少需要填写一组可用的网关配置。可以直接使用专用变量：

```dotenv
SLIME_DEEPSEEK_BASE_URL=https://your-provider.example/v1
SLIME_DEEPSEEK_API_KEY=your-api-key
SLIME_DEEPSEEK_MODEL=your-model
```

也可以填写 `SLIME_CODEX_*` 或 `SLIME_LLM_*` 作为共享回退配置。默认
`deepseek-harness-native` 使用 DeepSeek Chat Completions 协议，推理强度为
`max`；Codex、Pi 和 Claude 默认关闭，需要在 `dispatch.json` 中单独启用。
...
Agent Match 和 Benchmark 测试平台创建的项目固定使用 `hybrid`：Bootstrap 只负责建立初始事实，随后由 Reason 创建 Explore 分支；平台题目的历史项目保留创建时记录的启动模式。
...
| `tasks.reason.max_intents` | `2` | 一次 Reason 最多新建的 Intent 数 |
| `workers[].max_running` | DeepSeek Harness 为 `8` | 单个 Worker 适配器的并发上限 |
...
默认在 Dispatcher 启动和每次任务前执行模型探针（`startup_and_task`）。若上游短暂故障不应阻断调度，可改为 `startup_only` 或 `disabled`；任务调用错误仍会写入运行记录。
...
├─ protocol/                报告协议和提示词资源
└─ integrations/            Agent Match 与可选评测平台集成
```
...
- [Agent Match 混合模式验证记录](docs/verification-agent-match-hybrid-mode.txt)
- [DeepSeek Harness 验证记录](docs/verification-deepseek-harness.txt)
...
当前测试套件包含 250 项以上的单元与集成测试。提交代码前还应执行：
