...
- **统一入口**：Windows 使用 `slime.cmd`，Linux 使用 `slime`；部署由 Compose 完成。
...
以下示例在 Linux 使用 `./slime`。Windows 将其替换为 `.\slime.cmd`。

### 部署命令

```sh
./slime up                 # 启动 API 和 Dispatcher
