...
patch_file="docs/agent-match-hybrid-mode.patch"
test -s "$patch_file" || {
...
previous_image="slime-local:0.0.37-before-agent-match-hybrid"
...
if [ "${SLIME_SOURCE_ONLY_ROLLBACK:-0}" != "1" ] \
  ...
  printf '%s\n' 'Agent Match startup mode and API image restored to direct.'
else
  printf '%s\n' 'Agent Match source restored to direct; no local rollback image was present.'
fi
