...
STANDARD_PROFILE = WorkerProfile("standard", DEFAULT_WORKER_IMAGE, "none")
# Native model CLIs and remote CTF fixtures both need ordinary outbound
# connectivity.  The private ``slime-cairn-lab`` network is intentionally
...
class WorkerManager:
    ...
    def create_and_start(
        ...
        if executor(plan["create"]) != 0:
            raise RuntimeError("创建项目 Kali Worker 失败")
        if executor(plan["start"]) != 0:
            raise RuntimeError("启动项目 Kali Worker 失败")
