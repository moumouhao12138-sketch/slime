...
from .models import Intent, new_id
from .native_agent import NativeAgentConfig, NativeAgentMind, NativeBackendResolver
...
class AsyncDispatcher:
    ...
    async def _start_reason_if_ready(
        ...
        if not self.global_capacity.try_acquire(self.project_id, "reason"):
            return False
        worker = self.worker_pool.try_acquire("reason")
        if worker is None:
