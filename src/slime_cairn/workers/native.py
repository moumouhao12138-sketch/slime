...
'''
    ...
    def _extract_response(
        ...
        if not messages:
            ...
            def append_structured_error(value: Any) -> None:
                if not isinstance(value, dict):
                    return
                stop_reason = str(
                    value.get("stopReason") or value.get("stop_reason") or ""
                ).strip()
                errors = [
                    value.get("errorMessage"),
                    value.get("error_message"),
                    value.get("finalError"),
                    value.get("final_error"),
                ]
                found = False
                for error_value in errors:
                    ...
                    if isinstance(error_value, (dict, list)):
                        ...
                        detail = str(error_value).strip()
                    if detail:
                        diagnostics.append(
                            f"{stop_reason}: {detail}" if stop_reason else detail
                        )
                if not found and stop_reason.lower() in {"error", "failed"}:
                    diagnostics.append(f"stopReason: {stop_reason}")

            for event in events:
                append_structured_error(event)
                append_structured_error(event.get("message"))
                event_messages = event.get("messages")
                if isinstance(event_messages, list):
                    for message in event_messages:
                        append_structured_error(message)
